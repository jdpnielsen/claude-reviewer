...
# Rewrite the description (markdown, rendered in the web UI)
claude-reviewer update a1b2c3d4 --description "$(cat /tmp/pr-body.md)"
...
pass `--description ""` to clear it. The web UI renders it as markdown, so
...
commit SHA, so a rewritten history would otherwise reset all of them; `update`
re-points each one at the commit its SHA became. The mark itself still holds a
content hash from when it was made, so anything the rewrite genuinely changed
(a file's contents, a reworded message) goes back to unreviewed while
